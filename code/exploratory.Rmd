...
Covariate data matches the description presented by @Villela2013. However, there are two details worth 
mentioning. First, it appears that the aspect data was used by @Villela2013 as is, i.e. without linearising its
values. Linearisation is necessary because aspect is a circular variable, where its minimum (0º) and maximum
(360º) values have the same physical meaning. Linearisation can be done using the following equation:

$$northerness = |180 - aspect|$$

were northerness is a variable that indicates the degree of exposition of a surface to the north. I am not 
aware if and how this could affect the results of our study.

The second point is the presence of (possibly) spurious artefacts in the digital elevation model. This can be 
seen in the curvature and aspect data below. The first is polluted with noise resulting from the use of 
contour lines to generate the digital elevation model, i.e. the presence of stairs along the slope. This noise
is a typical feature of digital elevation models produced using contour lines. In general, the solution is to
use a smoothing filter to remove the noise from the digital elevation model before deriving terrain attributes.
...
Northerness data gives evidence of the presence of (possibly spurious) depressions in the digital elevation
model at places of high elevation. Analysing the northerness and curvature data, I believe that these 
depressions were created due to the presence of contour lines with erroneous elevation values at some of the 
higher parts of the landscape. As such, higher elevations may present high values of the topographic wetness 
index. However, these depressions in the digital elevation model could also represent true, existing 
depressions. I am not aware of the existence of depressions at higher elevations in the study area.

![Artefacts in elevation data as suggested by northerness data. The transect (red line) is shown in the next 
figure.](~/projects/urucu/res/fig/northerness_artefacts.png)

![Transect showing the change in elevation values in a depression.](~/projects/urucu/res/fig/sinks.png)

In both cases, I am not aware if and how these artefacts could damage the performance of our soil prediction 
models.
